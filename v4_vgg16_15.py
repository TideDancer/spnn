...
# Model
vgg = torch.load('pretrain/vgglike_15.pk')
n_layer = 14
...
mask_list.append(torch.ones(768,))
...
nclip_list = [28,40,76,76,160,152,160,544,608,384,384,416,576,304]
inc_list = [1,1,2,2,4,4,4,8,8,6,6,6,8,6]
# targets = [400, 250, 40]
...
cnt = 0
scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[3,  7, 10, 20], gamma=0.5)
forward_loss = 1000
test_acc = test(cnt)
while(test_acc < targetacc and cnt < 20 and forward_loss > 1e-5): 
  scheduler.step()
...
for epoch in range(100000):
    if epoch % 10 == 0: torch.save(net, 'checkpoint/v4_vgg16_15.pk')
    for layerid in range(n_layer):
        ...
        if 0 not in reverse_list and epoch > 0: 
          print('$$$$$$$$$$$$$$$$$$$$$$$$$ adjust $$$$$$$$$$$$$$$$$$$$$$')
          T = T + 5 if T < 21 else T
          inc_list = map(lambda e: e/2 if e > 1 else e, inc_list)
...
#post train
torch.save(net, 'checkpoint/v4_vgg_15.pk')
print("-------------- post train ------------")
optimizer = optim.Adam(net.vgg.parameters(), lr=lr/10)
